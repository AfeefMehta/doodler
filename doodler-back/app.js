...
io.on('connection', function(socket) {
    ...
    socket.on('create-lobby', function(data) {
        ...
        socket.emit('update-word-values', {words: generateWords(wordBank, data.lobbyNumWords)})
    ...
    socket.on('join-lobby', function(data) {
        ...
        // update player list and chat for client
        socket.emit('update-word-values', {words: ['Hidden', 'Hidden', 'Hidden']})
    ...
    socket.on('update-option-choice', function(data) {
        ...
        if (socket.id === lobby.players[lobby.currentPlayer].id && !lobby.optionPicked) {
            ...
            io.to(socketInfo[socket.id].lobbyName).emit('update-timer', {timeMessage: socketInfo[socket.id].username + " has " + lobby.currTime + " seconds left..."})
    ...
    socket.on('give-words', () => {
        ...
        if (socket.id === lobby.players[lobby.currentPlayer].id) {
            let words = generateWords(wordBank, lobby.numOptions)
            socket.emit('update-word-values', {words})
        } else {
            socket.emit('update-word-values', {words: ["Hidden", "Hidden", "Hidden"]})
        }
    })
